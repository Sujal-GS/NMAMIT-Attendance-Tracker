/**
 * React Bits - Beams Background Component (@react-bits/Beams-JS-CSS)
 * High-performance WebGL 3D animated light ribbons using Three.js
 */
...
/**
 * Initializes and mounts the Beams animated background
 * @param {HTMLElement} container - The container element to append the WebGL canvas
 * @param {Object} options - Configuration parameters matching React Bits Beams
 */
export function initBeamsBackground(container, options = {}) {
  ...
  const config = {
    ...
    beamColor: options.beamColor ?? '#000000',
  ...
  const initialZ = aspect < 1 ? 20 * Math.min(2.5, 1 / (aspect * 1.05)) : 20;
  ...
  group.rotation.set(0, 0, THREE.MathUtils.degToRad(config.rotation));
  group.add(mesh);
  group.add(dirLight);
  ...
  // Handle visibility / tab switching to save resources
  ...
  const onResize = () => {
    ...
    camera.position.z = aspect < 1 ? 20 * Math.min(2.5, 1 / (aspect * 1.05)) : 20;
  ...
  // Watch for theme toggle changes on <html>
  const themeObserver = new MutationObserver(() => {
    const isNowDark = document.documentElement.classList.contains('dark');
    beamMaterial.uniforms.uLightMode.value = isNowDark ? 0 : 1;
  });
...
// Auto-initialize when loaded as a module in document
if (typeof document !== 'undefined') {
  const setupBeams = () => {
    const container = document.getElementById('beams-canvas-container');
    if (container && !container.hasChildNodes()) {
      initBeamsBackground(container, {
        beamWidth: 2,
        beamHeight: 15,
        beamNumber: 12,
        lightColor: '#ffffff',
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', setupBeams);
  } else {
    setupBeams();
  }
}
